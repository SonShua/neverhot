...
def get_locations(request, location_name):
    """Make API call to openweathermap geocode.
    Fetch location details (latidude, longitude, country) matching location_name.
    ...
    url = f"{base_url}direct?q={location_name}&limit={search_limit_results}&appid={api_key}"
    city_created_list = []
    city_get_list = []
    nothing_found = False
    limit = True
...
def get_weather_forecast(slug):
    ...
    try:
        ...
        for x in range(0, 12):
            ...
            # Checks db a forecast for that city + datetime exists.
            # If yes -> update object. If no -> create object.
